...
func RegisterCorsHandlers(mux *http.ServeMux) *corsPolicyStore {
	...
	mux.HandleFunc("/api/2.22/buckets/cross-origin-resource-sharing-policies", store.handlePolicy)
	mux.HandleFunc("/api/2.22/buckets/cross-origin-resource-sharing-policies/rules", store.handleRules)
	return store
}
